...
export const abstinenciaAlcoolica: CasoClinico = {
  ...
  nos: [
    ...
    },
    {
      id: "etapa-3",
      pergunta:
        "Diante da evolução para desorientação, alucinações táteis e visuais ('gente mexendo na cortina'), tremor e hiperatividade autonômica, cerca de 48-72h após a última ingesta de álcool, qual é a hipótese diagnóstica mais provável e qual conduta deve guiar a intensidade do tratamento?",
      opcoes: [
        {
          texto:
            "Delirium tremens (forma grave da síndrome de abstinência alcoólica), tratado com benzodiazepínico em esquema fixo/front-loading guiado por sinais objetivos — não pela escala CIWA-Ar, que exige paciente comunicativo e não é válida neste paciente já confuso e desorientado",
          correta: true,
          explicacao:
            "Correto. A combinação de delirium (desorientação, alucinações, curso agudo), hiperatividade autonômica intensa e tremor, surgindo classicamente entre 48 e 96 horas após a última dose de álcool em paciente com consumo pesado e abrupta interrupção, define o delirium tremens — a forma mais grave e potencialmente fatal da síndrome de abstinência alcoólica. Uma vez o delirium já estabelecido, a escala CIWA-Ar deixa de ser válida: vários de seus 10 itens dependem do paciente relatar sintomas subjetivos (cefaleia, distúrbios sensoperceptivos, orientação), o que um paciente confuso não consegue fazer de forma confiável. O tratamento correto nesse estágio é esquema fixo (front-loading) de benzodiazepínico guiado por sinais objetivos e reavaliação clínica frequente — a terapia guiada por sintomas via CIWA-Ar é reservada à abstinência não complicada, em paciente comunicativo e cooperativo.",
          proximoNoId: "etapa-4",
        },
        {
          texto:
  ...
  pontosDeEnsino: [
    ...
    "A escala CIWA-Ar só é válida em paciente comunicativo, capaz de relatar sintomas subjetivos — não em delirium já estabelecido, como neste caso, onde o tratamento correto é esquema fixo (front-loading) de benzodiazepínico guiado por sinais objetivos.",
