...
export const manejoTabagismo: Fluxograma = {
  ...
  nodes: [
    ...
    },
    {
      ...
      detalhe:
        "Opções de primeira linha — a escolha considera preferência do paciente, comorbidades, contraindicações e disponibilidade. Vareniclina (agonista parcial do receptor nicotínico α4β2): entre as opções com maior eficácia isolada em metanálises em rede da Cochrane, superior à bupropiona e à TRN de agente único; iniciar 1-2 semanas antes da data de parada; efeito adverso mais comum é náusea; não consta na base de medicamentos deste Atlas, por isso sem link direto. Terapia de reposição de nicotina (TRN): adesivo transdérmico combinado a uma formulação de ação rápida (goma, pastilha, spray nasal ou inalador) tem eficácia comparável à vareniclina, superior à TRN de agente único. Bupropiona: eficácia estabelecida para cessação (ver ficha do medicamento), alternativa útil em depressão comórbida; história de convulsão é motivo para EVITAR a bupropiona — preferir TRN nesse cenário, com vareniclina como opção de cautela. Atenção em pacientes usando clozapina ou olanzapina: a fumaça — não a nicotina — induz a CYP1A2, então a cessação pode elevar os níveis séricos desses fármacos e exigir ajuste de dose.",
