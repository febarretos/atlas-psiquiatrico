...
async function chamarGemini(prompt: string, schema: unknown): Promise<string> {
...
export async function POST(request: NextRequest) {
  ...
  if (modo === "resposta-livre") {
    ...
    try {
      const casoLivre = await gerarCasoLivreUmaVez(prompt);
      return NextResponse.json({ casoLivre, inspiracao: paraFontes(inspiracao) });
    } catch (primeiroErro) {
      try {
        const casoLivre = await gerarCasoLivreUmaVez(prompt);
        return NextResponse.json({ casoLivre, inspiracao: paraFontes(inspiracao) });
        ...
        console.error("Falha ao gerar caso livre (2 tentativas):", primeiroErro, segundoErro);
        return NextResponse.json(
  ...
  try {
    const caso = await gerarMultiplaEscolhaUmaVez(prompt);
