...
export default function AssistenteMedicacao() {
  ...
  const [sintomasSelecionados, setSintomasSelecionados] = useState<Set<string>>(new Set());
  ...
  function recomecar() {
    setSintomasSelecionados(new Set());
  ...
  const candidatosDiagnostico = useMemo(() => {
    if (sintomasSelecionados.size === 0) return [];
    ...
    return diagnosticos
      ...
      .map((d) => {
        ...
        const matched = chave.filter((c) => sintomasSelecionados.has(c.id));
  ...
  const candidatosBase = useMemo<CandidatoBase[]>(() => {
    ...
    if (modo === "alvos") {
      return resultadosAlvos.map((r) => ({
        ...
        badgePrincipal: `${r.cobertura.length}/${alvosSelecionados.size} alvo(s) cobertos`,
    ...
    <div className="flex flex-col gap-8">
          ...
          {!diagnosticoEscolhido ? (
            <>
              <p className="text-sm text-slate-400">
                ...
                diagnósticos prováveis — ainda não fechou hipótese.
              </p>
                ...
                <div key={categoria}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sintomas
                      ...
                      .map((s) => {
                        const ativo = sintomasSelecionados.has(s.id);
              ...
              {sintomasSelecionados.size > 0 && candidatosDiagnostico.length === 0 && (
      ...
      {candidatosBase.length > 0 && (
        <div className="flex flex-col gap-6 border-t border-slate-800 pt-8">
          ...
          <div className="flex flex-col gap-4">
            {candidatosAvaliados.map(({ medicamento, badgePrincipal, detalhes, avaliacao }) => (
              <Link
                key={medicamento.id}
